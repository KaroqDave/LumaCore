...
HidWriteResult HidDeviceWriter::writeReports(const QString& path, const QVector<QByteArray>& reports) const
{
    ...
    if (device == nullptr) {
        hid_exit();
        return {false, 0, QStringLiteral("Could not open HID device path: %1").arg(path)};
    }
    ...
    for (int index = 0; index < reports.size(); ++index) {
        ...
        if (written < 0) {
            hid_close(device);
            hid_exit();
            return {false, totalWritten, QStringLiteral("hid_write failed for %1 on report %2.").arg(path).arg(index + 1)};
        }
        ...
        if (written != report.size()) {
            ...
            return {
                ...
                QStringLiteral("Short HID write for %1 on report %2: wrote %3 of %4 byte(s).")
                    .arg(path)
                    .arg(index + 1)
                    .arg(written)
                    .arg(report.size()),
...
) const
{
    ...
    if (written < 0) {
        hid_close(device);
        hid_exit();
        return {false, 0, {}, QStringLiteral("hid_write failed for request on %1.").arg(path)};
    }
    if (written != report.size()) {
        hid_close(device);
        hid_exit();
        return {
            ...
            qMax(0, written),
            {},
            QStringLiteral("Short HID request write for %1: wrote %2 of %3 byte(s).").arg(path).arg(written).arg(report.size()),
        };
    }
    ...
    if (read < 0) {
        return {false, written, {}, QStringLiteral("hid_read_timeout failed for %1.").arg(path)};
    }
    if (read == 0) {
        return {false, written, {}, QStringLiteral("Timed out waiting for HID response from %1.").arg(path)};
    }

...
ProbeResult probeHidapiDevices()
{
    ...
    for (hid_device_info* current = head; current != nullptr; current = current->next) {
        ...
        result.devices.append({
            ...
            stableProbeId(QStringLiteral("hid"), QStringLiteral("%1-%2-%3").arg(vid, pid, path)),
            product.isEmpty() ? QStringLiteral("HID device %1:%2").arg(vid, pid) : product,
    ...
    hid_exit();
    result.messages.append(QStringLiteral("hidapi discovered %1 device(s).").arg(result.devices.size()));
    return result;
#endif
}

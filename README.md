...
There are no published releases yet; both packages are produced from this repository.

- The portable Windows ZIP (`LumaCore-Windows-x64.zip`) is uploaded as a CI artifact by the Build workflow and can be built locally with `packaging\windows\package.ps1`. See [docs/windows-preview.md](docs/windows-preview.md) for running it, safe mock sessions, and current limitations.
- The Linux x64 staged install tarball (`LumaCore-Linux-x64.tar.gz`) is built locally following [docs/release-verification.md](docs/release-verification.md). See [docs/linux-package.md](docs/linux-package.md) for archive contents, safe mock testing, and install-style testing.
...
Build and stage Linux package artifacts with:
...
The native Windows preset configures into `build-windows/` while the Linux/WSL presets use `build/`, so the same checkout can hold both platform build trees at once without reconfiguring or deleting either.
...
- `docs/linux-package.md` documents Linux package archive contents, safe mock testing, and install-style testing.
...
- No packaged releases are published yet, and native installers and Linux distribution packages are not implemented; packaging currently produces a Windows portable ZIP and a Linux x64 staged install tarball.
